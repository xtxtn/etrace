...
static void show_stack_trace(__u64 *stack, int stack_sz, pid_t pid)
{
	...
	for (i = 0; i < stack_sz; i++) {
		...
		print_frame(sym->name, stack[i], sym->addr,
			    sym->offset, &sym->code_info);

		for (j = 0; j < sym->inlined_cnt; j++) {
			inlined = &sym->inlined[j];

			print_frame(inlined->name, 0, 0, 0,
				    &inlined->code_info);
		}
	}
...
static void usage(const char *prog)
{
    fprintf(stderr,
            ...
            "  -t 32  trace the arm32 (default: arm64)\n"
            "  -s         print stack addresses\n"
            "  -h         show this help\n"
            "\n"
            "Examples:\n"
            "  %s -p 1234 -t 32\n"
...
int main(int argc, char **argv)
{
    ...
    while ((opt = getopt(argc, argv, "p:c:e:t:hs")) != -1) {
        switch (opt) {
        case 'p': {
            char *end = NULL;
            long pid;
        ...
        case 's':
            config.print_stack_enabled = ENABLE_STACK;
            break;
